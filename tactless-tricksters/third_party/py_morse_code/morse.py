...
class DotDash:
    ...
    def __init__(self):
        ...
        self.dot_samples = self.get_beep(self.dot_duration)
        self.dash_samples = self.get_beep(self.dash_duration)
    ...
    def get_beep(self, duration):
        freq = 2 * np.pi * np.arange(self.fs * duration) * self.f / self.fs
        return np.sin(freq).astype(np.float32)
...
def morse_kwarg_validate(func):
    ...
    def validated(*args, **kwargs):
        try:
            ...
            raise TypeError('you must specify whether the message you '
                            'are passing in is in words or morse format!\n' +
...
class Morse:
    """ The Morse class can be used to translate messages
        between morse code and alphanumeric encodings.
        The class can also be used to play the morse code
        sounds associated with the message (all platforms)
        and read the message out loud (mac/linux only).

        A message can be read in when the class is initiated,
        or afterwards using the .read() function. The message
        must be passed in using one of the two keyword
        arguments: morse or words
