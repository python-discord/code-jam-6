...
class ConversationBubble(ButtonBehavior, MDCard):
    ...
    def _update_size(self, *_):
        ...
        if self.size_hint_y is None:
            for c in self.children:
                ...
                # Add a buffer to the bottom to prevent text clipping.
            self.height = temp_height + 10
    ...
    def init_morse_sounds(self, dt):
        ...
        if len(self.sound_list) > self.cur_sound_index:
            letter = self.sound_list[self.cur_sound_index]
            self.cur_sound = self.util.morse_helper.get_letter_as_morse_sound(letter)
